...
3. 对要验证的表单元素使用formChecker。

    $("form").formChecker(options)

   其中的options通常是一个对象，提供配置信息，详见下面的选项一节。另外，options也可以是字符串    ，目前支持'isValid'值，用于获取表单是否验证通过。
...
其中，key是要检查的具体元素的选择器，例如'input[name="name"]'。value是一个数组，数组的第一项是该字段的描述，如'姓名'；第二项是检查器的类型名，如'str'，详见后面的检查器一节；第三项是一个object，是供检查器使用的针对单个字段的配置选项，如{'maxLen': 6}，详见后面的检查器一节。

### global

说明：用于提供一些对全部字段适用的全局性配置，具体选项和单个字段的配置选项一样，详见后面的检查器一节。这些配置会被应用到formInput中每个字段中，但会被单个字段中的同名配置覆盖。
